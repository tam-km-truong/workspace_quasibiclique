...
def AB_V(rows_data, cols_data, edges, rate):
    """
    Arguments:
    ----------
...
def AB_E(rows_data, cols_data, edges, rate):
    """
    Arguments:
    ----------
    rows_data: list of tuples (row, degree) of rows in the matrix.
    ...
    LpProblem:
        The ILP model.
    """
    #rate = 0.0 # forcing for solving benchmarks 
    debug = 0 
    ...
    if debug >= 2:
        print()
        print('-' * 40) 
        print(f"******** Solving model AB_E ********", model.name, " with rate = ", rate)
        print('rate =', rate)
...
def AB_E_h(rows_data, cols_data, edges, rate):
    """
    Arguments:
    ----------
    rows_data: list of tuples (row, degree) of rows in the matrix.
...
def AB_E_r(rows_data, cols_data, edges, rate):
    """
    Arguments:
    ----------
    rows_data: list of tuples (row, degree) of rows in the matrix.
...
def __col_density_iff(rows_data, cols_data, edges, model, lpRows, lpCols, lpSum_col, lpSlack_col, rate):
    """
...
def __row_density_iff(rows_data, cols_data, edges, model, lpRows, lpCols, lpSum_row, lpSlack_row, rate):
    """
...
def __col_density(rows_data, cols_data, edges, model, lpRows, lpCols, rate):
    """
    Adds col density constraints to the model.
...
def __row_density(rows_data, cols_data, edges, model, lpRows, lpCols, rate):
    """
    Adds row density constraints to the model.
...
def zero_deleter(rows, cols, row_names, col_names, edges_1, nb_edges_0, iter, rate, debug): 
    ...
    * delta: error tolerance (percentage of accepted zeros) in the final result submatrix
    ...
    while density < density_threshold:
        ...
        if debug >= 1:
            ...
            print()
        rows_res, cols_res, edges_1_res, nb_edges_0_res, density = zero_deleter(rows_in, cols_in, row_names, col_names, edges_1_in, nb_edges_0_in, iter, rate, debug)
        rows_in = rows_res
        cols_in = cols_res
...
def exact(model_name, rows, cols, row_names, col_names, edges_1, delta, debug):     
    ...
    #     model = KP_QBr(rows, col_length, nb_eges_0, rate)
    # elif model_namel == 'KP_QBc':
    #     model = KP_QBc(cols, row_length, nb_eges_0, rate)  
    ...
    model = AB_E_r(rows_rem, cols_rem, edges_1_rem, rate=0.1)
    #model = AB_V(rows_rem, cols_rem, edges_1_rem, rate=0.1)
