...
def max_ones(
    ...
    print()
    print('-' * 40)
    print("row_threshold' value", row_threshold)
    print("col_threshold' value", col_threshold)
    print('-' * 40)
    print()
    prob += (row_threshold <= lpSum(row_choices[u] for u in range(bin_matrix.number_of_rows())
    )
    ),'row_threshold'
    prob += (col_threshold <= lpSum(column_choices[v] for v in range(bin_matrix.number_of_columns())
    )
    ),'col_threshold'
